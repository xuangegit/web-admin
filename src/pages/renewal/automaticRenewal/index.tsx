...
const AutomaticRenewal: React.FC = () => {
  ...
  const [autoConfig, setAutoConfig] = React.useState<any>({ open: false });
  return (
    <PageContainer>
      <ProCard>
        <div>
          自动续费规则{' '}
          <Button size="small" onClick={() => setNoticeModalOpen(true)}>
            设置通知
          </Button>
        </div>
        ...
        <ol className={styles.ol} style={{ marginTop: 10 }}>
          ...
          <li className={styles.li}>
            5、余额不足时不执行自动续费；平台会进行短信通知，请在收到短信后及时充值
          </li>
          <li className={styles.li}>
        ...
        <Flex gap={16} style={{ marginTop: 10 }}>
          <Button type="primary" onClick={() => setAutoConfig({ type: 1, open: true })}>
            开启自动续费
          </Button>
          <Button onClick={() => setAutoConfig({ type: 0, open: true })}>关闭自动续费</Button>
        </Flex>
      </ProCard>
      <SettingNoticeModal open={noticeModalOpen} onClose={() => setNoticeModalOpen(false)} />
      {autoConfig.open && (
        <AutoRenewalModal
          visible={autoConfig.open}
          onCancel={() => setAutoConfig({ open: false })}
        />
      )}
    </PageContainer>
  );
