...
const SettingNoticeModal:React.FC<NoticeProps> = (props:NoticeProps) => {
    ...
    return (
        <Modal title="自动续通知设置" open={open} onCancel={onClose} footer={false}>
            <div className={styles['notice-switch_wrapper']}> <label>自动续费通知：</label><Switch onChange={switchChange} checked={checked} /> </div>
        ...
        <div>
            <h3>通知设置</h3>
            <Flex vertical>
                <div>设置接收余额提醒的手机号 <Button type="link" onClick={goSetting}>去设置</Button></div>
                <div>绑定微信公众号接受提醒 <Button type='link' onClick={goSetting}>去绑定</Button></div>
            </Flex>
        </div>
        </Modal>
    )
